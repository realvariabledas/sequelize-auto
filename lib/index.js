          ...
          if (Object.keys(self.associationKeys[table]).length > 0) {
            ...
            for (const subTable of subTables) {
              ...
              for (const col of cols) {
                tsTableDef += 'readonly ' + self.associationKeys[table][subTable][col] + '?: ' + getPascalName(subTable) + '[];\n';
              }
            }
            tsTableDef += '\n';
            tsTableDef += 'static associations: {';
            for (const subTable of subTables) {
              const cols = Object.keys(self.associationKeys[table][subTable]);
              for (const col of cols) {
                tsTableDef += self.associationKeys[table][subTable][col] + ': Association<' + getPascalName(table) + ', ' + getPascalName(subTable) + '>;\n';
              }
            }
            tsTableDef += '}';
          }
