...
function AutoSequelize(database, username, password, options) {
  ...
  this.tables = {};
  this.foreignKeys = {};
  this.associationKeys = {};
  this.tablePrimaryKeys = {};
...
AutoSequelize.prototype.build = function (callback) {
  ...
  function mapForeignKeys(table, fn) {
    ...
    function assignColumnDetails(ref) {
      ...
      // console.log('foreignKeys', JSON.stringify(self.foreignKeys));

      if (ref.foreignSources !== undefined) {
        self.associationKeys[ref.foreignSources.target_table] = self.associationKeys[ref.foreignSources.target_table] || {};
        self.associationKeys[ref.foreignSources.target_table][table] = self.associationKeys[ref.foreignSources.target_table][table] || {};
        self.associationKeys[ref.foreignSources.target_table][table][ref.foreignSources.source_column] = getPascalName(ref.foreignSources.source_column) + getPascalName(table);
        self.tablePrimaryKeys[ref.foreignSources.target_table] = ref.foreignSources.target_column;
      }
...
AutoSequelize.prototype.run = function (callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      var tableName = self.options.camelCase ? getPascalName(table) : table;
      var tsTableDef = self.options.typescript ? '\n// table: ' + table + '\nexport class ' + getPascalName(tableName) + ' extends Model {' : '';
      if (i === 1 && self.options.typescript) {
        let localI = 0;
        for (const table of Object.keys(self.tables)) {
          localI++;
          const tableName = getPascalName(table);
          if (localI === 1) {
            tsTableDef = 'import { ' + tableName + 'ModelInitializer } from \'./' + tableName + '\';\n\n\n' + tsTableDef;
          } else {
            tsTableDef = 'import { ' + tableName + 'ModelInitializer } from \'./' + tableName + '\';\n' + tsTableDef;
          }
        }
        ...
        _.each(fieldAttr, function (attr, x) {
          ...
          if (attr === "foreignKey") {
            ...
            if (_attr === "boolean" || _attr === "bit(1)" || _attr === "bit") {
              ...
              val = 'DataTypes.INTEGER' + (!_.isNull(length) ? '({ scale: ' + length[0] + ' })' : ''); ``
      ...
      // console.log('solution keys', JSON.stringify(self.associationKeys));
      if (self.options.typescript) {
        if (self.associationKeys[table] !== undefined) {
          if (Object.keys(self.associationKeys[table]).length > 0) {
            tsTableDef += '\n\n';
            const subTables = Object.keys(self.associationKeys[table]);
            for (const subTable of subTables) {
              const cols = Object.keys(self.associationKeys[table][subTable]);
              for (const col of cols) {
                tsTableDef += spaces + 'public get' + self.associationKeys[table][subTable][col] + '!: HasManyGetAssociationsMixin<' + getPascalName(subTable) + '>;\n';
                tsTableDef += spaces + 'public add' + self.associationKeys[table][subTable][col] + '!: HasManyAddAssociationMixin<' + getPascalName(subTable) + ', number>;\n';
                tsTableDef += spaces + 'public has' + self.associationKeys[table][subTable][col] + '!: HasManyHasAssociationMixin<' + getPascalName(subTable) + ', number>;\n';
                tsTableDef += spaces + 'public count' + self.associationKeys[table][subTable][col] + '!: HasManyCountAssociationsMixin;\n';
                tsTableDef += spaces + 'public create' + self.associationKeys[table][subTable][col] + '!: HasManyCreateAssociationMixin<' + getPascalName(subTable) + '>;\n\n';

                tsTableDef += spaces + 'public readonly ' + self.associationKeys[table][subTable][col] + '?: ' + getPascalName(subTable) + '[];\n';
              }
            }
            tsTableDef += '\n';
            tsTableDef += spaces + 'public static associations: {\n';
            for (const subTable of subTables) {
              const cols = Object.keys(self.associationKeys[table][subTable]);
              for (const col of cols) {
                tsTableDef += spaces + spaces + self.associationKeys[table][subTable][col] + ': Association<' + getPascalName(table) + ', ' + getPascalName(subTable) + '>;\n';
              }
            }
            tsTableDef += spaces + '}\n';
          }
      ...
      text[table] += spaces + spaces + 'sequelize: this.dbConnection.connection,\n';
      if (self.options.paranoid) text[table] += spaces + spaces + 'paranoid: true,\n';
      if (self.options.paranoid) text[table] += spaces + spaces + 'timestamps: true,\n';
      ...
      let isLastOne = false;
      if (self.options.typescript) {
        if (_.keys(self.tables).length === i) {
          tsTableDef += '\n}\n\n';
          tsTableDef += 'export function SetupRelations() {\n';
          for (const table of Object.keys(self.tables)) {
            const primaryTableName = getPascalName(table);
            tsTableDef += spaces + 'DI.get<' + primaryTableName + 'ModelInitializer>(' + primaryTableName + 'ModelInitializer).initialize();\n';
          }
          tsTableDef += '\n';
          if (self.associationKeys !== undefined && self.associationKeys !== null) {
            for (const pkey of Object.keys(self.associationKeys)) {
              const primaryTableName = getPascalName(pkey);
              for (const ckey of Object.keys(self.associationKeys[pkey])) {
                for (const colKey of Object.keys(self.associationKeys[pkey][ckey])) {
                  isLastOne = true;
                  const childTableName = getPascalName(ckey);
                  tsTableDef += spaces + primaryTableName + '.hasMany(' + childTableName + ', {\n'
                  tsTableDef += spaces + spaces + 'sourceKey: \''+ self.tablePrimaryKeys[pkey] +'\',\n';
                  tsTableDef += spaces + spaces + 'foreignKey: \''+colKey+'\',\n';
                  tsTableDef += spaces + spaces + 'as: \''+self.associationKeys[pkey][ckey][colKey]+'\'\n';
                  tsTableDef += spaces + '});\n';
                }
              }
            }
        ...
        typescriptFiles[0] += tsHelper.def.getTableDefinition(tsTableDef, tableName, isLastOne);
      }
...
AutoSequelize.prototype.write = function (attributes, typescriptFiles, callback) {
  ...
  if (self.options.typescript) {
    if (typescriptFiles !== null && typescriptFiles.length > 1) {
      fs.writeFileSync(path.join(self.options.directory, 'dbModels.ts'), typescriptFiles[0], 'utf8');
      // fs.writeFileSync(path.join(self.options.directory, 'db.tables.ts'), typescriptFiles[1], 'utf8');
    }
  }

  function createFile(table, _callback) {
    let fileName = self.options.camelCaseForFileName ? getPascalName(table) : table;
